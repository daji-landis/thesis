...
contract test {  // enum for contract states.  I don't understand all the syntax here
    ...
    function verifierRandomness(bytes32  _verifierHash) public {
        // verifer commits randomness (don't forget '0x')
    ...
    function submitResult(uint _result, uint _proverPreimage) public{
        ...
        //randomness.verifierHash = keccak256(abi.encodePacked(_proverPreimage)); // add to check
    ...
    function setVerifyAddr(address _verifyAddr) public payable {
     ...
     // some sort of time out if the prover doesn't submit the proof in time
    function resolveVerify(Proof memory _proof, uint[2] memory input) public payable{
        // require(msg.sender == agreement.proverAddress);
        // proof = _proof;
        bool correctProof = false;
